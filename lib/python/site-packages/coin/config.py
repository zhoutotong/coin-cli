...
class Configration:
    ...
    def read_package(self, package):
        ...
        if os_tag not in pkg_list:
            coin.utils.warn(f'os tag {os_tag} not found in package list, user default instead')
            os_tag = 'default'
        
        if package not in pkg_list[os_tag]:
            if package not in pkg_list['default']:
                return None
            coin.utils.warn(f'package {package} not found in os tag {os_tag}, use default instead')
            os_tag = 'default'
